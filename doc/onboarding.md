...
- Model: local Meta Llama 3.1 8B snapshot documented in `docs/phase1/baseline.md`.
...
The complete final sweep command is preserved in [`docs/phase1/final-report.md`](../docs/phase1/final-report.md); it requires a CUDA GPU and is substantially more expensive than the smoke run.
...
- **Missing `vllm_flash_attn`:** install the validated wheel with `pip install --no-deps 'vllm-flash-attn==2.6.2'` in the project environment, as recorded in `docs/phase1/baseline.md`.
...
- **Need to inspect a failure:** check the run-specific `.log` file and the files under its `benchmark-results/<run-id>/` directory. Do not treat incomplete runs as experiment evidence.
