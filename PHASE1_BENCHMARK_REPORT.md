...
- `benchmark-results/phase1-low-load-poisson/`
...
PYTHONPATH="$PWD/swiftLLM:$PWD/swiftLLM/csrc" CUDA_VISIBLE_DEVICES=3 \
  ...
  --telemetry-interval-s 0.1 --output-dir benchmark-results \
  --run-id phase1-low-load-fixed --expected-num-gpu-blocks 3880
```
...
PYTHONPATH="$PWD/swiftLLM:$PWD/swiftLLM/csrc" CUDA_VISIBLE_DEVICES=3 \
  ...
  --seed 7 --telemetry-interval-s 0.1 --output-dir benchmark-results \
  --run-id phase1-low-load-poisson --expected-num-gpu-blocks 3880
```

## Evidence from real runs

Fixed run (`phase1-low-load-fixed`):
...
Poisson run (`phase1-low-load-poisson`):
