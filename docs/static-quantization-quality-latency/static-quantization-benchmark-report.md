# Static MorphServe quantization quality–latency benchmark report
...
Source evidence and the frozen pre-run specification are in [`static-quantization-benchmark-protocol.md`](static-quantization-benchmark-protocol.md). The machine-readable manifest is [`benchmark-results/static-quantization-quality-latency/protocol_manifest.json`](../../benchmark-results/static-quantization-quality-latency/protocol_manifest.json).

The final v2 workload uses the first request-timestamp-started 72-second half-open BurstGPT window with at least 100 rows: raw `[744286, 744358)`, 106 requests, and 1.75x time scaling. Scaled offsets span 0–124.25 seconds. Every request uses a fixed 1024-token prompt and 512 requested output tokens. The exact trace rows, DuReader records, prompts, and hashes are under `benchmark-results/static-quantization-quality-latency/input/`.
...
Values below are regenerated from raw request files by `benchmark.analyze_static_quantization_quality_latency`. Each condition has two independent runs; the table reports the run mean and min–max where useful.
...
- [`static_quantization_quality_latency_results.csv`](../../benchmark-results/static-quantization-quality-latency/static_quantization_quality_latency_results.csv)
- [`configuration_summary.csv`](../../benchmark-results/static-quantization-quality-latency/configuration_summary.csv)
- [`aggregate.json`](../../benchmark-results/static-quantization-quality-latency/aggregate.json)
- [`execution_commands.json`](../../benchmark-results/static-quantization-quality-latency/execution_commands.json)
- [`quality_vs_p95_ttft.png`](../../benchmark-results/static-quantization-quality-latency/quality_vs_p95_ttft.png)
- per-run `metadata.json`, `requests.jsonl`, `telemetry.jsonl`, `summary.json`, and `quality_metrics.jsonl` under `benchmark-results/static-quantization-quality-latency/runs/v2-*`.
...
PYTHONPATH=swiftLLM /nfs/home/s314511048/.venv/bin/python -m benchmark.prepare_static_quantization_workload \
  ...
  --output-dir benchmark-results/static-quantization-quality-latency/input
```
...
```bash
PYTHONPATH=swiftLLM /nfs/home/s314511048/.venv/bin/python -m benchmark.analyze_static_quantization_quality_latency \
  --manifest benchmark-results/static-quantization-quality-latency/protocol_manifest.json \
  --runs \
    benchmark-results/static-quantization-quality-latency/runs/v2-fp16_0-rep0 \
    benchmark-results/static-quantization-quality-latency/runs/v2-fp16_0-rep1 \
    benchmark-results/static-quantization-quality-latency/runs/v2-w4_8-rep0 \
    benchmark-results/static-quantization-quality-latency/runs/v2-w4_8-rep1 \
    benchmark-results/static-quantization-quality-latency/runs/v2-w4_16-rep0 \
    benchmark-results/static-quantization-quality-latency/runs/v2-w4_16-rep1 \
    benchmark-results/static-quantization-quality-latency/runs/v2-w4_32-rep0 \
    benchmark-results/static-quantization-quality-latency/runs/v2-w4_32-rep1 \
  --output-dir benchmark-results/static-quantization-quality-latency
```
