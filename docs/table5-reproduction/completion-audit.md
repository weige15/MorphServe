# Table 5 reproduction completion audit
...
|---|---|---|
| Evidence-backed specification constructed before benchmarking | `docs/table5-reproduction/experiment-spec.md`; paper SHA and Table 5 targets in `protocol_manifest.json` | PASS |
...
| Paper values treated as reference targets | `final-report.md` and `aggregate.json.paper_targets_are_reference_only=true` | PASS |
| F1 quality, P95 TTFT, and TTFT >2 s SLO defined | manifest protocol; `table5_analyze.py`; per-request `quality_metrics.jsonl` | PASS |
...
| Machine-readable manifest recorded before final runs | `benchmark-results/table5-reproduction/protocol_manifest.json`; final metadata hash matches it | PASS |
...
| Exact final command lines and logs saved | `benchmark-results/table5-reproduction/execution_commands.json` and 8 v2 log files | PASS |
...
| Four-row-equivalent result and quality-vs-P95 plot | `table5_results.csv`, `configuration_summary.csv`, `quality_vs_p95_ttft.png` | PASS |
| Derived values regenerated from raw artifacts | exact analyzer command in `final-report.md`; analyzer rerun during this audit | PASS |
...
| Final report has required separated sections | `final-report.md`: CONFIRMED FINDINGS, SUPPORTED BUT UNCERTAIN FINDINGS, BLOCKED QUESTIONS, REMAINING UNCERTAINTY | PASS |
