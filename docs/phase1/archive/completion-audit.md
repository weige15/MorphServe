...
| Repeat important knee-region points | `phase1-repeat-rps-32` and `phase1-repeat-rps-64`; repeat values retained and plotted | PASS |
...
| Compute required arrival/throughput/latency metrics | `sweep.csv` and report table contain target/achieved arrival, completed/token throughput, request counts, TTFT, queueing, and TPOT distributions | PASS |
| Compute queue/KV/swap/block/memory telemetry | `summary.json` `telemetry_stats`, `sweep.csv`, report telemetry table, and raw JSONL fields contain waiting/running/swapped stats, decoding/GPU blocks, logical KV, swaps/preemptions, and HBM stats | PASS |
...
| Generate required plots from saved data | `benchmark-results/phase1-saturation/plots/`: P95 TTFT, P95 queueing, peak logical KV, completed throughput, plus representative time series | PASS |
| Preserve representative time series below/near/above knee | Raw `telemetry.jsonl` paths for 4, 16, and 64 RPS and `representative_time_series.png` | PASS |
