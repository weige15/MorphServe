...
| `c20-short-control` | 50 / 128 tokens | `max_batch_size=32`, `max_tokens_in_batch=1024`, fixed 1000 RPS, 20 requests | 20/20 completed; peak 20 running, waiting 0, 220/3880 blocks = **5.67%**; no swaps. This is a fast low-pressure control, not KV saturation. TTFT mean 10.901 s, queueing mean 0.098 s, TPOT mean 0.02718 s. |
...
| **Selected: `selected-long-prompt-pressure`** | **2048 / 16 tokens** | **documented separate config below; fixed 100 RPS, 20 requests** | **13 resident requests, 7 waiting while pressure is high, 1677/1768 blocks = 94.85%; max batch is 32. No swap/preemption. Direct KV admission pressure.** |
...
PYTHONPATH="$PWD/swiftLLM:$PWD/swiftLLM/csrc" CUDA_VISIBLE_DEVICES=3 \
  ...
  --seed 2025 --telemetry-interval-s 0.25 \
  --output-dir benchmark-results/phase-1/kv-capacity-calibration/runs \
  --run-id selected-long-prompt-pressure-repro \
...
- `benchmark-results/phase-1/kv-capacity-calibration/runs/selected-long-prompt-pressure/`
- `benchmark-results/phase-1/kv-capacity-calibration/runs/repeat-long-prompt-pressure/`
...
The same command/configuration was repeated as `repeat-long-prompt-pressure`:
...
|---|---|---|
| Read required baseline, benchmark, saturation, audit, and reference documents | Files read before calibration: `baseline-configuration.md`, `archive/low-load-benchmark-harness-report.md`, `archive/short-workload-saturation-report.md`, `archive/short-workload-completion-audit.md`, `../../references/MORPHSERVE_PHASE1_REFERENCE.md` | PASS |
