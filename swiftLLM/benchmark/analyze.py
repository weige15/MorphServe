...
def _plot_data(points: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for y_key, filename in (
        ("ttft_s_p95", "p95-ttft-vs-offered-rps.png"),
        ("queueing_delay_s_p95", "p95-queueing-delay-vs-offered-rps.png"),
        ("peak_logical_kv_utilization", "peak-logical-kv-utilization-vs-offered-rps.png"),
        ("completed_request_throughput_rps", "completed-throughput-vs-offered-rps.png"),
    ):
        result[filename] = {
            "canonical": [
...
def _make_plots(
    ...
    plots: list[str] = []
    specs = (
        ("ttft_p95", "ttft_s_p95", "P95 TTFT (s)", "p95-ttft-vs-offered-rps.png"),
        ("queueing_p95", "queueing_delay_s_p95", "P95 queueing delay (s)", "p95-queueing-delay-vs-offered-rps.png"),
        ("kv_peak", "peak_logical_kv_utilization", "Peak logical KV utilization", "peak-logical-kv-utilization-vs-offered-rps.png"),
        ("throughput", "completed_request_throughput_rps", "Completed request throughput (RPS)", "completed-throughput-vs-offered-rps.png"),
    )
...
for spec in {sweep_specs}; do
  ...
  PYTHONPATH="$PWD/swiftLLM:$PWD/swiftLLM/csrc" CUDA_VISIBLE_DEVICES=3 \\
    ...
    --output-dir benchmark-results/phase-1/historical-short-workload/runs --run-id "$run_id" \\
...
PYTHONPATH="$PWD/swiftLLM" "$VENV/bin/python" -m benchmark.analyze \\
  ...
  --output-dir benchmark-results/phase-1/historical-short-workload \\
  --report docs/phase-1/archive/short-workload-saturation-report.md'''
    ...
    lines = [
        ...
        "",
        "This report is generated by `swiftLLM/benchmark/analyze.py` from the saved `metadata.json`, `requests.jsonl`, and `telemetry.jsonl` files. The raw request/system evidence is retained in each run directory; `rps-sweep-metrics.csv` and `sweep-summary.json` are derived artifacts. Independent timestamp-based percentile checks passed: **%s**." % ("PASS" if validation_passed else "FAIL"),
        "",
        ...
        "- Baseline scheduler audit: `scheduler.py` was hashed before and after the runs and compared to the vendored baseline; see `benchmark-results/phase-1/historical-short-workload/scheduler-audit.txt`.",
        ...
        "  --warmup-requests 1 --seed 2025 --telemetry-interval-s 0.1 \\",
        "  --output-dir benchmark-results/phase-1/historical-short-workload/runs --run-id warmup-calibration-fixed \\",
        "  --expected-num-gpu-blocks 3880",
        "```",
        "",
        ...
        "",
        "The complete machine-readable table, including every requested telemetry distribution and physical-memory statistic, is `benchmark-results/phase-1/historical-short-workload/rps-sweep-metrics.csv`. Repeated points are marked `is_repeat=true` in `sweep-summary.json` and the CSV.",
        "",
    ...
    for plot in plot_paths:
        lines.append(f"- `{plot}`")
    lines += [
        "- Derived summary: `benchmark-results/phase-1/historical-short-workload/sweep-summary.json`.",
        "- Derived table: `benchmark-results/phase-1/historical-short-workload/rps-sweep-metrics.csv`.",
        ...
        "- The explicit completion checklist is `docs/phase-1/archive/short-workload-completion-audit.md`; its final decision is based on the checks above rather than on test status alone.",
        "- Independent percentile recomputation from raw timestamps is recorded in `sweep-summary.json` under `verification` and passed for every run and TTFT/queueing/TPOT metric.",
...
def main() -> None:
    ...
    points.sort(key=lambda row: (float(row["target_rps"]), "repeat" in row["run_id"], row["run_id"]))
    ...
    _write_csv(args.output_dir / "rps-sweep-metrics.csv", points)
    ...
    print(json.dumps({"summary": str(args.output_dir / "sweep-summary.json"), "report": str(args.report), "plots": plot_paths, "verification": validation}, indent=2, sort_keys=True))
