...
def _make_plots(
    ...
    specs = (
        ("ttft_s_p95", "P95 TTFT (s)", "rps_vs_p95_ttft.png"),
        ("queueing_delay_s_p95", "P95 queueing delay (s)", "rps_vs_p95_queueing.png"),
        ("peak_logical_kv_utilization", "Peak logical KV utilization", "rps_vs_peak_logical_kv_utilization.png"),
        ("completed_request_throughput_rps", "Completed request throughput (RPS)", "rps_vs_completed_throughput.png"),
    )
    ...
    plot_data["rps_vs_ttft_components.png"] = component_series
    ...
    axis.legend()
    figure.tight_layout()
    path = output_dir / "rps_vs_ttft_components.png"
    figure.savefig(path, dpi=160)
    plt.close(figure)
    plots.append(str(path))
...
for spec in {sweep_specs}; do
  ...
  PYTHONPATH="$PWD/swiftLLM:$PWD/swiftLLM/csrc" CUDA_VISIBLE_DEVICES=3 \\
    ...
    --seed {metadata['random_seed']} --telemetry-interval-s {metadata['telemetry_interval_s']} --output-dir benchmark-results \\
...
PYTHONPATH="$PWD/swiftLLM" "$VENV/bin/python" -m benchmark.final_analyze \\
  ...
  --report docs/phase1/final-report.md'''
    ...
    lines += [
        ...
        "```",
        "",
        f"The canonical points are `{', '.join(f'{point['target_rps']:g}' for point in canonical)} RPS`; repeats are explicitly marked by their run IDs. The run directories are listed in `benchmark-results/phase1-final/aggregate.json` and each contains raw request and telemetry JSONL.",
        "",
    ...
    lines += [
        ...
        "## Verification",
        "",
        f"- Raw-run and independent timestamp/percentile verification: **{'PASS' if validation['passed'] else 'FAIL'}**; details are in `aggregate.json`.",
...
def main() -> None:
    ...
    write_csv("sweep.csv", points)
    write_csv("decomposition.csv", decomposition_rows)
    write_csv("admission_evidence.csv", admission_rows)
    write_csv("representative_time_series.csv", representative_rows)
    (args.output_dir / "request_audit.json").write_text(json.dumps(request_audit, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    validation = {
        ...
        "plot_source_matches_points": all(
            ...
            for filename, key in (
                ("rps_vs_p95_ttft.png", "ttft_s_p95"),
                ("rps_vs_p95_queueing.png", "queueing_delay_s_p95"),
                ("rps_vs_peak_logical_kv_utilization.png", "peak_logical_kv_utilization"),
                ("rps_vs_completed_throughput.png", "completed_request_throughput_rps"),
            )
    ...
    print(json.dumps({"aggregate": str(args.output_dir / "aggregate.json"), "report": str(args.report), "plots": plots, "verification": validation}, indent=2, sort_keys=True))
