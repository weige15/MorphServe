"""Run one frozen v9 runtime-static or closed-loop MorphServe condition."""
...
SOURCE_PATHS = (
    ...
    "swiftLLM/benchmark/run_closed_loop_condition.py",
    "swiftLLM/benchmark/prepare_closed_loop_workloads.py",
    "swiftLLM/benchmark/run_closed_loop_plan.py",
    "swiftLLM/benchmark/analyze_closed_loop.py",
    "swiftLLM/benchmark/audit_closed_loop.py",
    "swiftLLM/benchmark/test_closed_loop_controller.py",
)
...
def validate_workload(workload: list[dict[str, Any]]) -> None:
    if len(workload) not in (64, 192):
        raise ValueError(f"frozen workload must contain 64 or 192 requests, got {len(workload)}")
...
async def run(args: argparse.Namespace) -> Path:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "5":
        raise RuntimeError("the frozen v9 protocol requires CUDA_VISIBLE_DEVICES=5")
    ...
    metadata: dict[str, Any] = {
        "schema_version": 1,
        "experiment": "closed_loop_runtime_v9",
        "run_id": args.run_id,
        "planned_run_id": args.planned_run_id,
        "attempt_index": args.attempt_index,
        "condition": args.condition,
        "workload_class": workload[0].get("v9_workload_class"),
    ...
    controller: SustainedCompoundController | None = None
    ...
    try:
        ...
        if args.condition == "closed_loop_dynamic":
            controller = SustainedCompoundController(measurement_start_ns)
