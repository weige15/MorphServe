...
def _fwd_paged_attention_phase1(
    ...
    k_extension_ptrs = k_cache_extension + (cur_layer*num_kv_heads+my_kv_head_id)*block_size*head_dim + tl.arange(0, block_size)[:, None]*head_dim + tl.arange(0, head_dim)[None, :]
    v_extension_ptrs = v_cache_extension + (cur_layer*num_kv_heads+my_kv_head_id)*block_size*head_dim + tl.arange(0, block_size)[:, None]*head_dim + tl.arange(0, head_dim)[None, :]
    ...
    if my_start_token_idx + seq_block_size > my_seq_len:
        ...
        for block_i in range(0, my_num_blocks):
            ...
            if block_index < base_num_blocks:
                ...
                extension_index = block_index - base_num_blocks
                k_block = tl.load(k_extension_ptrs + extension_index*num_layers*num_kv_heads*block_size*head_dim)
                v_block = tl.load(v_extension_ptrs + extension_index*num_layers*num_kv_heads*block_size*head_dim)
