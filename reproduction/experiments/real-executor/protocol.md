...
- Issue atomic recovery actions; snapshot allocator/group/cache metadata, remove groups and restore in LIFO order 26→24→25 only after all blocks are free. If restoration fails, reattach the exact snapshot; if compensation fails, poison the executor/coordinator and stop serving.

## Gate

1. Injected expansion failure leaves exact FP16 bytes/logits, `num_blocks`, `num_free_blocks`, free bitmap, cache-list lengths, group size and active/event state; records reverse rollback.
