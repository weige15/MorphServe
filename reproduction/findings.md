...
2. **Mechanism correctness exposed and then isolated a release-blocking lifetime defect.** One synthetic reclaimed region produced correct bounded K/V views, zero allocator-byte increase, writable physical storage, and same-base restoration. Released source segfaulted at exit because static C++ maps retained `py::object` metadata past Python finalization. A separate native-metadata reconstruction now passes the identical gate and exits 0; multi-region Triton and serving behavior remain open.
