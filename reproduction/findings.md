...
4. **Real local W4 is available, but not in the candidate's released format.** AutoAWQ executes 224 packed GEMMs with 3.626 GB decoder storage including metadata. Its eight-way split-K Triton kernel is not bit deterministic (repeat relative L2 up to 0.00493) though top-5 is stable. Candidate code instead expects a public llm-awq class signature and `scaled_zeros/.pt` state that do not match either public llm-awq or the local `qzeros/safetensors` asset.
