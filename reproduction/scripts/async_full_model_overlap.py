...
def main():
    ...
    # Compile both precision paths outside all timed regions.
    model.forward([ids],[7],[],ignore_kvcache=True,return_logits=True).cpu()
    executor.morph_to_w4([a.layer]); model.forward([ids],[7],[],ignore_kvcache=True,return_logits=True).cpu()
    ...
    gate={"repeat_count":len(w4)==len(fp16)==a.repeats,"host_enqueue_nonblocking":all(not row['ready_at_enqueue_return'] for row in rows),"same_history_w4_within_envelope":all(row['comparison']['relative_l2']<0.005 for row in w4),"same_history_fp16_exact":all(row['comparison']['relative_l2']==0 for row in fp16),"overlap_observed":all(row['overlap_ms']>0 for row in rows),"final_fp16_bytes_exact":restored_exact,"final_state_fp16":executor.active_layers==[]}
