...
CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP:$ROOT/runtime/candidate-python:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/async_full_model_overlap.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json" --repeats 3
EOF
