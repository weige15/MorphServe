...
PYTHON_KERNELS="$VENDOR/MorphServe"
VENV="$ROOT/.venv"
TMP="$ROOT/results/tmp/candidate-kv-mapping"
OUT="$ROOT/experiments/candidate-kv-mapping/results"
...
CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP/csrc:$ROOT/runtime" MORPHSERVE_CANDIDATE_PYTHON="$PYTHON_KERNELS" MORPHSERVE_TEST_OUTPUT="$OUT/metrics.json" "$VENV/bin/python" -m unittest "$ROOT/tests/test_candidate_kv_mapping.py" -v
EOF
...
MORPHSERVE_CANDIDATE_PYTHON="$PYTHON_KERNELS" \
MORPHSERVE_TEST_OUTPUT="$OUT/metrics.json" \
  "$VENV/bin/python" -m unittest "$ROOT/tests/test_candidate_kv_mapping.py" -v \
