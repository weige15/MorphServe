...
TMP="$ROOT/results/tmp/autoawq-layer-switch"
OUT="$ROOT/experiments/autoawq-layer-switch/results"
...
CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/autoawq_layer_switch.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json"
EOF
...
PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" \
  "$VENV/bin/python" "$ROOT/scripts/autoawq_layer_switch.py" \
