...
CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP/csrc:$ROOT/runtime" MORPHSERVE_TEST_OUTPUT="$OUT/metrics.json" "$VENV/bin/python" -m unittest "$ROOT/tests/test_candidate_memory_manager.py" -v
EOF
...
PYTHONPATH="$TMP/csrc:$ROOT/runtime" \
MORPHSERVE_TEST_OUTPUT="$OUT/metrics.json" \
  "$VENV/bin/python" -m unittest "$ROOT/tests/test_candidate_memory_manager.py" -v \
