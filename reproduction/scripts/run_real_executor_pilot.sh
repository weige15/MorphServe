...
VENV="$ROOT/.venv"; LOCK="$ROOT/configs/fp16-requirements-lock.txt"; CSRC="$ROOT/runtime/candidate-csrc"; PYTHON_SRC="$ROOT/runtime/candidate-python"
TMP="$ROOT/results/tmp/real-executor"; OUT="$ROOT/experiments/real-executor/results"
...
git -C "$ROOT/.." status --short -- reproduction/runtime reproduction/scripts/real_executor_pilot.py reproduction/scripts/run_real_executor_pilot.sh reproduction/configs/fp16-requirements-lock.txt > "$OUT/source-status.txt"
...
CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/real_executor_pilot.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json"
EOF
...
CUDA_VISIBLE_DEVICES="$GPU" PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/real_executor_pilot.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json" > "$OUT/run.stdout.log" 2> "$OUT/run.stderr.log"
