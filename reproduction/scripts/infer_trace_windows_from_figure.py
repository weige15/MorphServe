...
def main():
    ...
    result={"schema_version":1,"classification":config['classification'],"method":config['extraction'],"azure_code":azure(a.azure_code,az,config['azure']['minimum_nonzero_source_bins']),"burstgpt_1_v1.1":burst(a.burst_v1,bu,config['burst']['minimum_nonzero_source_bins']),"limitations":["Plot digitization is approximate and partially occluded by legends.","PDF section 5 says the evaluation uses the representative 72-second snippets in Figure 1, but integer-bin matching cannot recover a sub-second boundary exactly.","Correlation identifies arrival-shape candidates, not the unpublished downsampling seed/operation or request-to-context mapping.","Candidates are frozen before serving outcomes but are not explicit author-published offsets."]}
