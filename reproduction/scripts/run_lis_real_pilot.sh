...
VENV="$ROOT/.venv"; LOCK="$ROOT/configs/fp16-requirements-lock.txt"
CSRC="$ROOT/runtime/candidate-csrc"; PYTHON_SRC="$ROOT/runtime/candidate-python"
TMP="$ROOT/results/tmp/lis-real-pilot"; OUT=${MORPHSERVE_TEST_RESULT_DIR:-"$ROOT/experiments/lis-real-pilot/results"}
LAYERS=${MORPHSERVE_LIS_LAYERS:-29,30,31}
...
mkdir -p "$OUT" "$ROOT/results/tmp"; rm -rf "$TMP"; mkdir -p "$TMP"; cp -a "$CSRC" "$TMP/csrc"
...
HF_DATASETS_OFFLINE=1 HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/lis_real_pilot.py" --fp16-model "$FP16" --w4-model "$W4" --layers "$LAYERS" --output "$OUT/metrics.json"
EOF
...
HF_DATASETS_OFFLINE=1 HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES="$GPU" PYTHONDONTWRITEBYTECODE=1 \
PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" \
 "$VENV/bin/python" "$ROOT/scripts/lis_real_pilot.py" --fp16-model "$FP16" --w4-model "$W4" --layers "$LAYERS" --output "$OUT/metrics.json" > "$OUT/run.stdout.log" 2> "$OUT/run.stderr.log"
run_rc=$?; set -e; end_ns=$(date +%s%N); echo "$run_rc" > "$OUT/run.exitcode"
...
cat "$OUT/run.stdout.log"; cat "$OUT/run.stderr.log" >&2; exit "$run_rc"
