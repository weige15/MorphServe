...
TMP="$ROOT/results/tmp/lis-real-pilot"; OUT="$ROOT/experiments/lis-real-pilot/results"
...
HF_DATASETS_OFFLINE=1 HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/lis_real_pilot.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json"
EOF
...
PYTHONPATH="$TMP/csrc:$PYTHON_SRC:$ROOT/runtime" \
 "$VENV/bin/python" "$ROOT/scripts/lis_real_pilot.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json" > "$OUT/run.stdout.log" 2> "$OUT/run.stderr.log"
