...
CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$TMP/csrc:$ROOT/runtime/candidate-python:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/multirequest_ownership.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json"
EOF
...
uv venv --clear --python python3.12 "$VENV" > "$OUT/venv.log" 2>&1; uv pip install --python "$VENV/bin/python" -r "$LOCK" > "$OUT/install-base.log" 2>&1; uv pip install --python "$VENV/bin/python" autoawq==0.2.9 zstandard > "$OUT/install-autoawq.log" 2>&1
...
start=$(date +%s%N); set +e; CUDA_VISIBLE_DEVICES="$GPU" PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$TMP/csrc:$ROOT/runtime/candidate-python:$ROOT/runtime" "$VENV/bin/python" "$ROOT/scripts/multirequest_ownership.py" --fp16-model "$FP16" --w4-model "$W4" --output "$OUT/metrics.json" > "$OUT/run.stdout.log" 2> "$OUT/run.stderr.log"; rc=$?; set -e; end=$(date +%s%N); echo "$rc" > "$OUT/run.exitcode"
