...
| H12 | §4.3 | Complete W4 layer swap ≈6 ms and fully hidden by decoding | CUDA timeline with distinct morph/decode streams and exposed stall | **Modified-condition partial, numerical claim unverified**: candidate remains blocking; independent persistent-stream copier queues 4 MiB in 0.222 ms while prior use is unfinished and event-safe tests pass, but full-layer/decode timeline is pending GPU headroom |
...
| H24 | §4.3; Appendix C | Real in-place W4/FP16 decoder-layer swapping, pinned variants, precompiled GEMM | Packed-weight storage including metadata, stable GPU base address, numerical execution, event/lifetime tests | **Approximate/modified-condition (partial)**: real AutoAWQ layer runs from same FP16 base with zero allocator delta and exact restore; persistent-stream copier, prepared wrapper variants and just-in-time event barrier pass small-region tests; full-model async rerun/overlap remains unverified |
| H25 | §4.4; Appendix C | Reclaimed weight bytes become physically usable, non-contiguous KV blocks | Address-bound checks, mapping oracle, capacity expansion/shrink/content preservation | **Approximate/modified-condition repaired**: explicit fallback fixes vendor misrouting; two real request IDs occupy reclaimed IDs 4/5, unsafe shrink refuses while preserving content, and succeeds after real frees; timed concurrent decode pending |
| H26 | §4.1 controller | Persistent pressure causes coordinated morph/resize and recovers without oscillation | Frozen mode configs and synthetic pressure tests | **Approximate/modified-condition real integration partial**: real W4/KV `[25,24,26]`, injected rollback, arbitrary mapping, LIFO recovery, FCFS preservation, and occupied-group deferral across two request IDs pass; no timed arrivals/decode concurrency |
...
| H28 | State preservation | Morphing needs no flush, re-prefill, restart, KV quantization, or KV eviction | Repeated prefill/decode adaptations against same-precision-history oracle | **Approximate/modified-condition strong partial**: active same-history request survives switching/migration; separate two-request ownership test refuses occupied shrink, preserves sentinel/progress/FCFS, and restores after frees with zero preemptions; concurrent serving pending |
