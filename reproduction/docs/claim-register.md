...
| H25 | §4.4; Appendix C | Reclaimed weight bytes become physically usable, non-contiguous KV blocks | Address-bound checks, mapping oracle, capacity expansion/shrink/content preservation | **Approximate/modified-condition repaired**: vendor fused `[25,24,26]` path misroutes into layer 23; independent explicit-region fallback writes layer 26 and matches dense attention error 0, while descending fast path remains exact; performance/integration pending |
| H26 | §4.1 controller | Persistent pressure causes coordinated morph/resize and recovers without oscillation | Frozen mode configs and synthetic pressure tests | **Approximate/modified-condition real integration partial**: reconstructed accuracy mode drives real W4/KV groups `[25,24,26]`, injected expansion rollback, explicit mapping and LIFO recovery with exact FP16 final state; no active workload/concurrency yet |
