...
| H11 | §4.3 | Llama 2 7B transfer ≈4 ms W4, ≈16 ms FP16 on PCIe Gen4 26–28 GB/s | CUDA events for H2D bytes; distinguish transfer/reconstruction/stall | **Unverified**; local RTX 3090 is modified hardware |
| H12 | §4.3 | Complete W4 layer swap ≈6 ms and fully hidden by decoding | CUDA timeline with distinct morph/decode streams and exposed stall | **Unverified**; candidate code synchronizes transfer stream |
| H13 | Appendix A | Full 32-layer LIS sequence under 15 minutes on one GPU | Timed conditioned-MDS profiler excluding model init, exact model/calibration identified | **Unverified** |
...
| H23 | §4.2; Algorithm 1 | Conditioned greedy MDS, LIS weights 0.25/0.25/0.5, `argmax` | Unit tests that fail for static/unconditioned MDS; saved calibration and profile | **Partially reproduced**: selection/history/validation pass 4 tests in `tests/test_profiling.py`; model metric collection and real profile remain unverified |
| H24 | §4.3; Appendix C | Real in-place W4/FP16 decoder-layer swapping, pinned variants, precompiled GEMM | Packed-weight storage including metadata, stable GPU base address, numerical execution, event/lifetime tests | **Tested-not-reproduced (partial)**: packed synthetic bytes reuse stable base, but candidate copies immediately synchronize and an explicit in-flight race corrupts restored bytes; AWQ numerical execution/precompile/overlap remain unverified |
| H25 | §4.4; Appendix C | Reclaimed weight bytes become physically usable, non-contiguous KV blocks | Address-bound checks, mapping oracle, capacity expansion/shrink/content preservation | **Approximate/modified-condition (partial)**: physical reclaim plus two-tail prefill/decode/attention and five synchronized cycles pass; fixed-stride assumption remains; uncoordinated restore corrupts 1,530 bytes after an in-flight write |
| H26 | §4.1 controller | Persistent pressure causes coordinated morph/resize and recovers without oscillation | Frozen mode configs and synthetic pressure tests | **Unimplemented** as specified; candidate policy lacks modes/smoothing/persistence |
...
| H28 | State preservation | Morphing needs no flush, re-prefill, restart, KV quantization, or KV eviction | Repeated prefill/decode adaptations against same-precision-history oracle | **Tested-not-reproduced (lifetime subclaim)**: five synchronized synthetic cycles preserve bytes, but restore lacks an in-flight event barrier and a delayed reclaimed-KV write corrupts restored weights; full request state remains unverified |
| H29 | Baseline integrity | No-morph FP16 preserves SwiftLLM behavior | Token/logit parity and scheduler trace | **Partially supported outside this workspace** by prior local SwiftLLM research; must rerun here against selected source |
