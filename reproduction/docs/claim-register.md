...
| H25 | §4.4; Appendix C | Reclaimed weight bytes become physically usable, non-contiguous KV blocks | Address-bound checks, mapping oracle, capacity expansion/shrink/content preservation | **Mixed / tested-not-reproduced for arbitrary order**: descending tails and one active request pass, but measured LIS prefix `[25,24,26]` makes group 2 overwrite layer 23 instead of 26 due fixed-stride mapping; real integration blocked pending repair |
