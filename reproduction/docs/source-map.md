...
| Offline LIS profiling | Section 4.2; Appendix A; Algorithm 1 | No profiling implementation, saved sequence, calibration config, or profile file in candidate repository | Missing. Reconstruction required. |
...
| KV physical capacity expansion | Section 4.4; Appendix C | C++ `MemoryManager::acquire_new_kvcache`; `update_kvcache_after_replacing_layer`; `BlockManager` capacity extension | Present as storage views into the tail of overwritten FP16 layer regions. Needs executed address-bound and allocator-ownership tests. |
| Non-contiguous block mapping | Section 4.4; Appendix C | Triton kernels in `worker/kernels/kvcache_mgmt.py` and `paged_attn.py`; extended block IDs map into reclaimed layer regions | Present in source. Uses pointer arithmetic from the first reclaimed region to later regions; safety and content preservation are unverified. |
| Safe shrink/restore | Section 4.4 | `if_kv_cache_unused`; `quant2org_and_update_kvcache` | Partial. Only the last appended reclaimed region can be removed, and restoration is refused if its blocks are occupied. In-flight CUDA use/event lifetime is not checked. |
