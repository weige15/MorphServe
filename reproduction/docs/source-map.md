...
| Safe shrink/restore | Section 4.4 | `if_kv_cache_unused`; `quant2org_and_update_kvcache` | **Negative event-safety evidence.** Occupancy check covers only the last appended region; no in-flight event is checked. A delayed separate-stream write completed after restore and corrupted 1,530 bytes. Explicit wait control and five synchronized cycles were exact. |
