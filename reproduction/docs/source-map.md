...
| Morphing Controller | Section 4.1; example thresholds KV >85%, queue delay >100 ms; Section 5 modes | Reconstruction `controller.py`, `integration.py`, `real_executor.py`; frozen `configs/reconstructed-controller-modes.json` | Historical accuracy-mode pilot morphs `[25,24,26]`, grows 4→1849 blocks, rolls back injected failure, and recovers LIFO. Follow-up atomic recovery/poisoning repairs pass CPU tests but await real-GPU rerun. Thresholds reconstructed; no active workload/concurrency. |
