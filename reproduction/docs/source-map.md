...
| Morphing Controller | Section 4.1; example thresholds KV >85%, queue delay >100 ms; Section 5 modes | Reconstruction `controller.py` + `integration.py`; candidate scheduler incomplete | 6 policy and 5 fake-executor tests cover modes, persistence, FCFS preservation, action ordering and rollback. Values beyond examples are reconstructed; real GPU engine integration pending. |
...
| Non-contiguous block mapping | Section 4.4; Appendix C | Candidate combined Triton kernels derive later pointers from group 0; multi-kernel alternative exists but is not default | Descending two-tail oracle passes, but measured LIS `[25,24,26]` deterministically writes group 2 into layer 23. Arbitrary profile order is **not reproduced** and blocks real controller integration until explicit pointer/remap repair. |
