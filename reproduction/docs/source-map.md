...
| Offline LIS profiling | Section 4.2; Appendix A; Algorithm 1 | Reconstruction `runtime/morphserve/profiling.py`; real collector `scripts/lis_real_pilot.py`; frozen profile under `profiles/` | Three-layer real WikiText-2/2048 pilot retains LTS/LRS, executes 6 conditioned MDS calls, saves `[29,30,31]`, exact restore. Full 32-layer paper profile remains missing. |
