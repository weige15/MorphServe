...
| Real AWQ W4 execution | Section 4.3; Appendix C | Candidate targets `awq.quantize.qmodule.WQLinear`; local adapter baseline uses AutoAWQ `WQLinear_GEMM` | Candidate public API/checkpoint format is mismatched/unrunnable. Modified static AutoAWQ executes 224 real packed modules and 3.626 GB decoder bytes including metadata; exact-repeat logits fail from split-K atomics. In-place adapter remains pending. |
