...
The 4-MiB CUDA seam requires negligible capacity, so it was rerun on the occupied GPU solely for current-revision correctness. Five tests passed at source revision `696afde...` with an empty source-status sidecar: host enqueue returned while prior use was unfinished, bytes/address were exact, the model consumed the ready event immediately before the affected layer, the async path accumulated no redundant native events, and partial expansion rollback published its barrier under the new preflight invariants. Contended timings (0.312-ms host enqueue, 0.571-ms CUDA-event interval) are supporting diagnostics only. Next step remains the four full-model current-revision runners when safe headroom reappears.
