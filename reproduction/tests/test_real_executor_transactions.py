...
class FakeCopier:
    def __init__(self,fail_source=None): self.fail_source=fail_source; self.calls=[]
    def enqueue(self,layer,source,size,tensor_map):
        self.calls.append((layer,source))
        if source==self.fail_source: raise RuntimeError('injected copy failure')
...
def bare_executor():
    ...
    executor.active_layers=[]; executor.quant_objects={}; executor.kv_groups=[]; executor.log=[]; executor.layer_stride_bytes=8
